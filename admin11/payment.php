...
<form name="paymentview" method="post" action="del_payment1.php" onSubmit="return fundel()">
<center>
...
<tr>
	<td>
		<?php
			...
			echo "<tr><th>All Delete</th><th>Id</th><th>Card No</th><th>Card Type</th><th>Expiry Date</th><th>Action</th></tr>";
			while($row=mysql_fetch_array($res))
			{
				echo "<tr><td><input type='checkbox' name='paymentdel[]' value=".$row[0]."></td>";
				echo "<td>$row[0]</td>";
				echo "<td>$row[1]</td>";
				echo "<td>$row[2]</td>";
				echo "<td>$row[3]</td>";
				echo "<td><a href='del_payment.php?id=$row[0]'>Delete</a></td>";
				echo "</tr>";
			}
...
<script>
	...
	function checkpro()
	{
		...
		{
			if(window.document.forms[0].element[a].name=="paymentdel[]")
			{	
